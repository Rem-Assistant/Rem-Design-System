...
| `PermissionsOnboardingView` | `Screen/Onboarding — Permissions` `551:49` | built from stale RemUI (no callers); founder didn't name it, but same provenance as the retired How-Rem-Works | 🔶 **verify/founder** — the shipping app requests permissions via system prompts, not a value-prop screen; likely folds into `Proposed · Onboarding`. Left in place pending founder call. |
