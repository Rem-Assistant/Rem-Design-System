...
| Execution trace (pattern reference / modal variant) — "In progress" pill + X, title/subtitle/time, MAIN/SUBAGENT sections, step rows (✓/✗ + title + description + chevron), "Working" footer. **The product surface for this pattern is now View history `413:32`** (task-scoped, pushed); this specimen stays as the modal/standalone reference (e.g. opened from the agent avatar / in-chat). | `431:21` | agent step-timeline / "show your work" surface |
...
| View history — Activity (task-scoped execution trace) | `413:32` | Task & Events page — **MERGED with the execution-trace pattern** (founder-approved, 2026-09-24). Pushed screen: back+"Activity" nav → task header (title + subtitle + timestamp, **no modal pill/X**) → MAIN/SUBAGENT sections of step rows (status icon ✓/✗ + bold title + gray description + chevron) → "Working" footer. Replaces the old one-comment-deep Rem/You activity list. Entered from the task detail (and the agent avatar routes here). Backing view `TaskActivityHistoryView` to evolve toward this step timeline. |
...
| Onboarding — Setting Up (deploying) | `415:15` | ⚠ **UP FOR DEPRECATION** (founder) — `OnboardingFlow.deployingContent`. Don't invest further. |
