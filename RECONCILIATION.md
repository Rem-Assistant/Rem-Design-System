...
|---|---|---|---|
| `InitialOnboardingView` (sign-in) | `Screen/Login` `411:15` | maps to shipping `OnboardingFlow.signIn` (caller `ContentView:46`) | ✓ **verified 2026-09-25** — logo + "Rem" + "Turn your thoughts into actions" + Continue-with-Google/Apple black buttons + exact legal footer all match `OnboardingFlow.swift`. Minor: logo is a plain blue tile vs the network-glyph mark (cosmetic). |
| `ValuePropOnboardingView` ("How Rem Works", 3 steps) | ~~`545:40`~~ → **Retired** | founder 2026-09-25: **"this How Rem Works screen is deprecated"**; built from stale RemUI (no callers) | ⌦ **RETIRED** — moved to Archive/Retired, dimmed. Onboarding future = `Proposed · Onboarding` (Connect apps → Set up voice), per the gateway-migration contract. |
| `PermissionsOnboardingView` | ~~`551:49`~~ → **Retired** | built from stale RemUI (no callers); same provenance as the retired How-Rem-Works | ⌦ **Retired (reversible, pending founder confirm)** — applied the founder's established rule to its last identical case. Real permissions live in **Settings→Permissions `349:905`** (has a caller); the shipping app also uses system prompts. If the future onboarding (`Proposed · Onboarding`) reframes a permissions primer, rebuild it there. Trivially un-retired if the founder wants it kept. |
| "Unblock Rem's power" (founder: "also old") | — | searched the whole Figma file: **no such frame** | n/a in Figma — a RemUI-only concept; RemUI already demoted to non-binding inspiration. |
...
| `HistoryView` | **Chat Sessions** `438:15` | RemUI History = a *placeholder* ("Chat history"); shipping `ChatHistoryView` = the real sessions list `438:15` | ✓ **verified 2026-09-25** vs `ChatHistoryView.swift` — "Chat Sessions" title, "Search conversations" search, session rows (title + preview + timestamp) all match |
| `SettingsView` | Settings `130:44` | shipping `SharedSettingsView` | ✓ **verified 2026-09-25** — section-for-section: Profile · **"Your agent runtime" → Rem·Connected** · Billing & Usage · Permissions · About · Share/Send Feedback/Report a Bug · Sign Out · Delete. ⚠ the **agent-runtime section is the gateway watch-list item** (deprecates with the migration's Product-cleanup slice → founder call #2 below). |
| `TaskEventView` | Task detail `299:2` · New Task `404:5` | RemUI (title circle + Badges/DateInfo/AlertRepeat/Notes) ≈ shipping | ✓ **verified 2026-09-25** vs `TaskEventView.swift` — title, `DateInfoSection` (clock + date), Duration/No alert/No repeat badges (`EditableAlertRepeatSection`), "Add your notes here" (`notesSection`), Last Activity + Run now/View history + Continue-in-chat all match |
| `TaskInspectorSheet` | `Screen/Task Inspector (sheet)` `581:61` | **real shipping sheet** (`TaskEventView.swift:961`, `.sheet` + `.presentationDetents([.medium,.large])`, opened from the date/alert row). RemUI's version is a subset. | ✓ **built** faithful to `taskSections`: Clear/Task Details/Done toolbar + insetGrouped cards — §1 **Any Time** toggle + **Start Date** (tinted date pills), §2 **Alert**/**Repeat** menu pickers (`chevron.up.chevron.down`), §3 **Duration** button (value + `chevron.right`). Chrome is a faithful grabber+toolbar; optional follow-up = re-base on kit Inspector sheet `493:3199` instance. |
