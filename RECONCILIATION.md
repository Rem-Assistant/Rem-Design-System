...
| `InboxView` | `Screen/Inbox` `206:703` | crux settled → rows follow **shipping `SharedTaskRow`**, not RemUI. But the Figma rows (`TaskEventRow` instances `311:3…`) **drift from `SharedTaskRow.swift`**: they carry a leading **calendar-badge-plus icon** (no such icon in code) and a **dashed circle**; code renders just a **solid `circle`** (`U+100000`, size 18, secondary) + title (**headline**, 2-line) + optional metadata pills. `showTimeIndicator:false` in Inbox ⇒ no time column. | 🔶 **fix `TaskEventRow`** → drop the calendar-badge-plus icon, solid `circle`, headline title + pills. **Shared component — used by Agenda too**, so the fix re-flows both; confirm before editing. |
